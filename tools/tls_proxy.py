...
class Proxy(BaseHTTPRequestHandler):
    ...
    def _take_upstream(self):
        """Dùng lại TLS connection để request kế tiếp không phải handshake lại."""
        with _pool_lock:
            while _pool:
                conn = _pool.pop()
    ...
    def _forward(self, method):
        ...
        # Upstream có thể đóng một connection đang nằm trong pool. GET an toàn
        # để thử lại đúng một lần bằng connection mới. POST không retry vì mất
        # response không chứng minh server chưa nhận body.
        for attempt in (0, 1):
            conn = None
            try:
                ...
                if conn is not None:
                    conn.close()
                # Không retry POST mù quáng: timeout không chứng minh server
                # chưa nhận assignment/action.
                if method == "POST" or attempt == 1:
                    self.send_response(502)
                    self.send_header("Content-Length", "0")
                    self.end_headers()
                    with _lock:
                        sys.stderr.write(
                            f"[proxy] loi upstream: {type(exc).__name__}\n"
                        )
                    return
